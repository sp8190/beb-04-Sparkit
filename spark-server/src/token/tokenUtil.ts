...
const TOKEN_CONTRACT_DEPLOYED_OWNER_ADDRESS =
  '0x3dD51a69aA684EC7CbE2CC2C075311aA872C7A4B'
...
export async function getUserBalance(userAccount: String) {
  ...
  const contract = new web3.eth.Contract(SITokenABI, TOKEN_CONTRACT_ADDRESS, {
    from: TOKEN_CONTRACT_DEPLOYED_OWNER_ADDRESS,
    gas: GAS_FEE,
  })
  return await contract.methods.balanceOf(userAccount).call()
}

export async function sendTokenToWriter(
  ...
  await contract.methods
    .transfer(writerAccount, INCENTIVE_TOKEN_AMOUNT)
    .send({ from: TOKEN_CONTRACT_DEPLOYED_OWNER_ADDRESS, gas: GAS_FEE })
...
export async function voteToOtherUser(
  ...
  //디코딩
  const decryptedPrivateKey = require('aes256').decrypt(
    process.env.PRIVATE_KEY_SECRET,
    senderPrivateKey,
  )
  web3.eth.accounts.wallet.add(decryptedPrivateKey)
  await contract.methods
    .transfer(receiverAccount, VOTING_TOKEN_AMOUNT)
    .send({ from: senderAccount, gas: GAS_FEE })
